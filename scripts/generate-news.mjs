...
async function fetchNewsRss(query, locale) {
  ...
  return arr
    ...
    .filter((item) => !/joomla/i.test(`${item.title} ${item.link}`))
    .slice(0, 14);
}
...
function enrichSummary(topic = {}, sources = []) {
  ...
  return `${main} ${sourceLine}参照URLで一次情報を確認できる。`.replace(/\s+/g, ' ').trim();
}
